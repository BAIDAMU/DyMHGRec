...
import torch
import time
from Hyp_models import HypDrug
...
resume_path = "./saved/Hyp/Epoch_16_TARGET_0.06_JA_0.6038_DDI_0.04501.model"
...
# evaluate
def eval(model, data_eval, voc_size, epoch, pretrained_model, diag_eval, pro_eval, med_eval):
    model.eval()
    ...
    for step, input in enumerate(data_eval):
        ...
        for adm_idx, adm in enumerate(input):
            target_output, _, _= model(input[: adm_idx + 1], pretrained_model, diag_eval, pro_eval, med_eval, step)
    ...
    if args.Test:
        ...
        for _ in range(10):
            ...
            ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med = eval(
                model, test_sample, voc_size, 0, pretrained_model, diag_test,pro_test,med_test
            )
    ...
    for epoch in range(EPOCH):
        ...
        for step, input in enumerate(data_train):
            ...
            for idx, adm in enumerate(input):
                ...
                result, loss_ddi, sim = model(seq_input, pretrained_model, diag, pro, med, step)      # 新增diag, pro, med,返回sim
        ...
        tic2 = time.time()
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med = eval(
            model, data_eval, voc_size, epoch, pretrained_model, diag_eval, pro_eval, med_eval
        )
        print(
