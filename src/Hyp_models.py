...
from layer import EmbeddingLayer, hypergraph_part, one_hypergraph
...
class HypDrug(nn.Module):
    ...
    def forward(self, input, pretrained_model, diag, pro, med, step):
        ...
        for admission in input:
            ...
            i1, i2 = self.hypergraph_encode(d_seq, p_seq, d_embeddings, p_embeddings, pretrained_model)
            # code_i1, code_i2 = self.pair_graph_encode(d_seq, p_seq, d_embeddings, p_embeddings, pretrained_model)

            diag_seq.append(i1)
            proc_seq.append(i2)
            # code_diag_seq.append(code_i1)
            # code_proc_seq.append(code_i2)
        ...
        for idx, adm in enumerate(input):
            if len(input) <= 1 or idx==0:
                ...
                adm[2] = input[idx - 1][2][:]
                m_seq = adm[2]
                med_1 = self.one_hypergraph_encode(m_seq, m_embeddings, pretrained_model)
            med_seq.append(med_1)

        diag_seq = torch.cat(diag_seq, dim=1)  # (1,seq,dim)
        proc_seq = torch.cat(proc_seq, dim=1)  # (1,seq,dim)
        med_seq = torch.cat(med_seq, dim=1)  # (1,seq,dim)
